    ...
    <Fragment>
      {projects.slice(0, 6).map((project, index) => (
        ...
        >
          ...
          >
            <Link 
              href={optimizeCloudinary(project.images[project.default_image_index])} 
              target="_blank" 
              className="group/zoom relative block overflow-hidden rounded-3xl"
            >
              <Image
                src={optimizeCloudinary(project.images[project.default_image_index])}
                alt={project.title}
                fill
                quality={100}
                unoptimized={true}
